...
const param_name_str = 
			[ [ "temp", "min" ], [ "temp", "max" ], [ "pressure" ], [ "humidity" ], 
			  ...
			  [ "snow" ], [ "weather", "icon" ] ];
const openweathermap_address = 'http://api.openweathermap.org/data/2.5/onecall?lat=49.99&lon=36.23&exclude=current,minutely,hourly,alerts&units=metric&appid=25446dc6c2ea52216ff635d00e0fcca9';

const myMain = async (global_path, allow_output) => {
  // 1 Получить прогноза
  const response = await axios.get(openweathermap_address);
  // 2 Получит текщее время прогноза (информационно, для формирования строки ошибок со временем)
...
export const MainForec = async (global_path, allow_output) => {
  await myMain(global_path, allow_output);
  goOffline(database);
}

//MainForec();
export const today_forecast_recorded = async (global_path) => {
